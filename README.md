...
Each step presents a **numbered task sequence** for moving through the work. Optional tasks are labeled, and adaptive scientific conversation comes from your configured model. **What carries forward** exposes saved documents from earlier steps. **Project materials** opens the shared source library, literature claim ledger, datasets and runs, consistency records, and project history without changing the selected research step.
...
Both extended demos use the **same seven step titles and numbered tasks as the live app**, generated from the shared workflow definitions. Each step shows its project title, optional help, the saved research document, and what carries forward. Only the protocol and final report have required supervisor checkpoints; the extra reflections and discussions are optional. The new logo appears in both demo headers.

**Education — Maya:** explore study habits, distinguish quiz performance from learning, question self-reported measurements, retain conflicting literature, correct a duplicate and minutes-to-hours conversion, and reject causal and universal claims.

**Business — Alex:** separate a sponsor's ROI request from a revenue question, examine manager selection and skills transfer, resist inflated transaction-level sample sizes, correct dollars versus kUSD, document sponsor pressure for selective analysis, and align an executive summary with the full report.

Each case includes:

- Seven interactive decision exercises with consequences for all three choices, plus the original learning questions.
- Four inspected fictional source excerpts and a six-record simulated screening log covering a preprint, duplicate, unavailable full text and qualitative counterpoint.
- Three simulated public-data candidates illustrating incompatible units, restricted access and context-only use. No catalogue searches or repository downloads occur in the demo.
- Four downloadable synthetic CSVs: the primary extract, a raw register, separate contextual aggregates and an exploratory subset. A downloadable Node preparation script reproduces the primary extract from the raw register, preserving missingness and documenting conversions and exact-duplicate removal.
- Two separately approved, actually computed R runs: primary R1 and a post-hoc R2 omitting the highest complete exposure. Both retain inputs, hashes, scripts, counts, intervals, residuals, figures and environments. The exploratory check is not presented as an independent replication or proof of robustness.
- Authored revision histories, prospective design checks, later consistency findings, responses and renewed final review.

Maya's primary regression uses six of eight synthetic records; R2 uses five of seven. Alex's primary uses twelve of fourteen, with an interval crossing zero; R2 uses eleven of thirteen. Both preserve two incomplete records and their exclusions. These deliberately small invented datasets illustrate process, not adequate sample sizes or real findings. All conversations, searches, sources and supervisor decisions are authored simulations. Numerical results and the preparation check execute only when maintainers build the demos, not when visitors open them.

Download the walkthrough, primary CSV, dataset workspace or reproduction bundle. The data step also offers each CSV and the preparation script separately. These are practice materials, **not importable project backups**. To practice in the live app, upload the CSVs into a separate project and review your own analysis plans. A live project backup is exported through **Export project** and restored through **Import project** as a separate copy. Browsing either demo preserves the active project and unsaved edits. Progress is separate per demo and can be reset with **Restart walkthrough**; this expanded edition starts fresh browser progress.

Maintainers regenerate the four R runs with `npm run build:demo`, without accessing the active notebook or model settings. Base narratives live in `lib/demo-case.mjs` and `lib/business-demo-case.mjs`; `lib/demo-depth.mjs` adds the current task mapping and advanced scenarios; `lib/demo-preparation.mjs` defines reproducible synthetic preparation. The served files are `public/demo-case.json` and `public/business-demo-case.json`. Numerical placeholders for both cases are resolved from actual R outputs. Unit tests check current task alignment, preparation, hashes and independently calculated slopes. Run `node scripts/demo-smoke.mjs` and `node scripts/demo-smoke.mjs --business` with Playwright installed (or `PLAYWRIGHT_MODULE` configured) to verify tasks, decision exercises, downloads, separate progress and notebook isolation.
...
Step 2 offers **Find literature for your research interest (optional)** before paper inspection. The agent can generate search terms, retrieve actual records from Crossref, Europe PMC and arXiv, and suggest a reading order with reasons. arXiv manuscript versions are labeled separately from journal records. You can also search without AI, save a reading list, and upload papers you already have. Discovered records remain separate from inspected evidence. See the [literature discovery guide](docs/literature-discovery.md).
...
1. Use **Manage your project data workspace** in Step 4 to keep multiple uploaded CSVs and shortlisted public records together. Attach downloaded Dataverse CSVs to their source record, add organizational notes, and choose a stored file for inspection and analysis. Limits: 20 files, 20 MB total, 2 MB per file, 2–10,000 rows and 1–50 uniquely named columns. The app records a SHA-256 of each stored UTF-8 CSV and profiles numeric values and missingness. Source attribution is researcher supplied, and files are not automatically merged.
