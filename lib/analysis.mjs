...
const relevant = ['question', 'design', 'data'];
const versions = (p) =>
  relevant.map((id) => ({ id, version: p.milestones.find((m) => m.id === id).version }));
export const planCurrent = (p, plan) =>
  plan.contextVersions.every((s) => p.milestones.find((m) => m.id === s.id).version === s.version);
...
export function importDataset(p, a) {
  p.datasets ||= [];
  if (p.datasets.length >= 5)
    throw new WorkflowError('This notebook supports up to five dataset versions.');
...
export function approvePlan(p, a) {
  ...
  if (!plan || !planCurrent(p, plan))
    throw new WorkflowError(
      'The plan is outdated. Create a new plan for the current question, design and data report.',
      409,
    );
...
export function analysisRunSummary(p) {
  return (p.analysisRuns || [])
    ...
    .map((r) => ({
      id: r.id,
      current: r.contextVersions.every(
        (s) => p.milestones.find((m) => m.id === s.id).version === s.version,
      ),
