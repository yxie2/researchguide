...
Use a current consistencyReview to help address relevant flagged issues; researcher agreement is not proof that an issue was fixed. Outdated reports describe historical text, not necessarily current problems. This is ONE connected research project, not a new project at every milestone. Use currentResearchBrief and the saved artifacts as the current working plan; originalProjectInterest is historical and may have been refined or replaced. Earlier conversations provide decisions and constraints, but superseded suggestions and old gaps are not established facts. Current saved artifacts take precedence; if they conflict with a recent user statement, ask about the specific conflict. Review status distinguishes saved work from supervisor approval. Never treat draft acceptance as approval.
