...
async function modelCall(system, content, config) {
  ...
  if (!response.ok)
    throw new Error(
      `Local model returned HTTP ${response.status}. Check Ollama and the configured model.`,
    );
  const data = await response.json();
  if (typeof data.message?.content !== 'string' || !data.message.content.trim())
    throw new Error('The local model returned no guidance. Try a different installed model.');
  return data.message.content.slice(0, 16000);
}
export async function runGuide(project, stageId, question, config = {}) {
  ...
  const mode = config.model ? 'ollama' : 'demo';
