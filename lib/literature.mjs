...
export function validLiteratureUrl(value) {
  try {
    ...
    return (
      ...
      !u.username &&
      !u.password &&
      ['doi.org', 'europepmc.org'].includes(u.hostname)
    );
  } catch {
    return false;
  }
...
export async function searchLiterature(query, fetcher = fetch) {
  ...
  const names = ['Crossref', 'Europe PMC'];
  ...
  for (const response of responses)
    if (response.status === 'fulfilled')
      for (const r of response.value) {
        const key = r.doi ? r.doi.toLowerCase() : r.title.toLowerCase();
...
async function assess(record, context, config, model) {
  const raw = await model(
    ...
    JSON.stringify({ context, papers: record.results }),
    config,
  );
  let data;
  try {
