...
export function validateProject(p) {
  ...
  if (p.isDemo !== undefined && typeof p.isDemo !== 'boolean') fail('demo flag');
  ...
  for (const search of p.literatureSearches || []) {
    ...
    list(search.results, 'literature results', (paper) => {
      ...
      if (
        paper.arxivUrl !== undefined &&
        (!validLiteratureUrl(paper.arxivUrl) || new URL(paper.arxivUrl).hostname !== 'arxiv.org')
      )
        fail('arXiv URL');
      if (
...
export async function savedProjects(dataDir, active) {
  ...
  const deleted = new Set(
    names.filter((n) => /^deleted-[a-f0-9-]{36}\.json$/i.test(n)).map((n) => n.slice(8, -5)),
  );
  const found = new Map(
    deleted.has(active.id) ? [] : [[active.id, { project: active, active: true }]],
  );
...
export const isDemoProject = (p) =>
  p.isDemo === true || ['maya-first-study', 'alex-business-study'].includes(p.id);
export function editableProject(p) {
  if (isDemoProject(p))
    throw new WorkflowError('Demo projects are read-only and cannot be renamed or deleted.', 403);
  if (!uuid.test(p.id)) throw new WorkflowError('Invalid project ID.');
}
export function renamedProject(p, title) {
  editableProject(p);
  if (typeof title !== 'string' || !title.trim() || title.length > 160)
    throw new WorkflowError('Enter a project title of 1–160 characters.');
  ...
  next.events.push({
    ...
    type: 'renamed',
    message: `Project title changed from “${p.title}” to “${next.title}”.`,
  });
  return next;
}
export async function archiveProject(dataDir, p) {
  if (!uuid.test(p.id)) throw new WorkflowError('Invalid project ID.');
  const file = path.join(dataDir, `${p.id}-r${p.revision}.json`);
  await writeFile(file + '.tmp', JSON.stringify(p, null, 2));
  await rename(file + '.tmp', file);
}
export async function projectDeleted(dataDir, id) {
  if (!uuid.test(id)) return false;
  try {
    await access(path.join(dataDir, `deleted-${id}.json`));
    return true;
  } catch (error) {
...
export async function trashProject(dataDir, p) {
  editableProject(p);
  // Soft deletion: preserve a complete recovery copy, then atomically hide all revisions.
  const folder = path.resolve(dataDir, 'trash', p.id);
  if (!folder.startsWith(path.resolve(dataDir) + path.sep))
    throw new WorkflowError('Invalid trash location.');
  await mkdir(folder, { recursive: true });
  await writeFile(path.join(folder, 'project.json'), JSON.stringify(p, null, 2));
  const marker = path.join(dataDir, `deleted-${p.id}.json`);
  await writeFile(
    marker + '.tmp',
    JSON.stringify({ id: p.id, title: p.title, deletedAt: new Date().toISOString() }),
  );
